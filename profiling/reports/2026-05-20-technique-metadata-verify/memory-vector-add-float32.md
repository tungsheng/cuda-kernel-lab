...
- Benchmark command: `uv run benchmark-memory --backend triton --device cuda --op vector_add --numel 16777216 --dtype float32 --block-size 1024 --warmup 5 --iterations 10 --skip-correctness --output experiments/results/aws-ec2/2026-05-20-technique-metadata-verify-profiled/memory-vector-add-float32.jsonl`
- JSONL result: `experiments/results/aws-ec2/2026-05-20-technique-metadata-verify-profiled/memory-vector-add-float32.jsonl`
...
| --- | ---: | --- | --- |
| Kernel time | 418688 | ns | `gpu__time_duration.sum` |
| DRAM throughput | 92.22 | % | `gpu__dram_throughput.avg.pct_of_peak_sustained_elapsed` |
| DRAM bytes read | 154474368 | byte | `dram__bytes_read.sum` |
| DRAM bytes written | 76960640 | byte | `dram__bytes_write.sum` |
| Occupancy | 81.68 | % | `sm__warps_active.avg.pct_of_peak_sustained_active` |
